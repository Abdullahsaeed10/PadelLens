...
with st.form("log_match", clear_on_submit=True):
    ...
    with sc3:
        # Set 3 is only used if first two are split
        split = (set1_us > set1_them) != (set2_us > set2_them)
        a3, b3 = st.columns(2)
        with a3:
            set3_us = st.number_input("Set 3 · us", 0, 7, value=0,
                                      disabled=not split,
                                      help="Only fillable if sets 1-2 are split")
        with b3:
...
if submitted:
    if us_sets == them_sets:
        st.error("Score isn't decisive. Please complete the score (one side must win 2 sets).")
        ...
        row = {
            ...
            "set3_us": set3_us if split else "",
            "set3_them": set3_them if split else "",
            "result": result,
        ...
        append_to_csv(row)
        st.cache_data.clear()
        st.success(f"✓ Match #{new_id} saved as **{result}** "
                   f"({us_sets}-{them_sets} in sets). "
                   f"Head to **My Stats** to see your updated dashboard.")
